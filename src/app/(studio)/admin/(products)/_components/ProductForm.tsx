...
export default function ProductAddForm({ product, categories }: ProductProps) {
  ...
  const onSubmit = async (data: ProductInput) => {
    ...
    try {
      ...
      const finalShortDesc = data.shortDescription || rawDescription.replace(/<[^>]+>/g, '').slice(0, 160) || "Product Description";
    ...
    <div>
      <form onSubmit={handleSubmit(onSubmit)}>
        <div className="flex flex-col gap-5 mb-5">
          ...
          <div className="grid grid-cols-1 gap-5 lg:grid-cols-2">
            {/* Short Description */}
            {/* Short Description (Hidden/Removed - populated from Description) */}
            {/* <Controller
              control={control}
          ...
          <Controller
            ...
            rules={{
              validate: (value) =>
                ...
                "At least one thumbnail with an image is required.",
            }}
            render={({ field }) => (
              <>
                <div>
